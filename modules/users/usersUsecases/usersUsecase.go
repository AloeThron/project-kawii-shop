...
import (
	...
	"github.com/AloeThron/project-kawii-shop/packages/kawiiauth"
...
func (u *usersUsecase) GetPassport(req *users.UserCredential) (*users.UserPassport, error) {
	...
	// Sign token
	accessToken, err := kawiiauth.NewKawiiAuth(kawiiauth.Access, u.cfg.Jwt(), &users.UserClaims{
		Id:     user.Id,
		RoleId: user.RoleId,
	})
	refreshToken, err := kawiiauth.NewKawiiAuth(kawiiauth.Refresh, u.cfg.Jwt(), &users.UserClaims{
		Id:     user.Id,
		RoleId: user.RoleId,
	})
...
func (u *usersUsecase) RefreshPassport(req *users.UserRefreshCredential) (*users.UserPassport, error) {
	// Parse token
	claims, err := kawiiauth.ParseToken(u.cfg.Jwt(), req.RefreshToken)
	if err != nil {
		return nil, err
	}
	...
	accessToken, err := kawiiauth.NewKawiiAuth(
		kawiiauth.Access,
		u.cfg.Jwt(),
		newClaims,
	)
	...
	refreshToken := kawiiauth.RepeatToken(
		u.cfg.Jwt(),
		newClaims,
